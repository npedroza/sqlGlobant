...
csv_file_path="employees_big.csv"
with open(csv_file_path, 'r') as file:
    data_df = pd.read_csv(file, header=None)
...
### Append registry of 100 new users
### I changed only the ID the names are the same and the other columns
print("Appending 100 new employees")
start_time = time.time()
csv_file_path="employees_forappend.csv"
with open(csv_file_path, 'r') as file:
    data_df = pd.read_csv(file, header=None)
    data_df.columns = ['id_emp','name', 'date', 'id_dep', 'id_job']
data_df.to_sql('employees',  engine, index=False, if_exists='append')
elapsed_time = time.time() - start_time
print("Elapsed time:", elapsed_time, "seconds for inserting data into Tables")

print("Appending 1000 new employees")
start_time = time.time()
csv_file_path="employees_forappend1000.csv"
with open(csv_file_path, 'r') as file:
    data_df = pd.read_csv(file, header=None)
    data_df.columns = ['id_emp','name', 'date', 'id_dep', 'id_job']
data_df.to_sql('employees',  engine, index=False, if_exists='append')
